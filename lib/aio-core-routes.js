...
{"reasons": ["short reasons the answer gave, if any"], "competitors": ["names of businesses OTHER THAN \\"${brandName}\\" that the answer recommends or mentions"]}`;
      ...
      const failure = publicProviderError(error, { provider: 'Gemini', operation: 'The live audit', setupPath: 'Settings → Your connections → Gemini' });
      logger.error('[AIO Audit API] Grounded audit failed:', failure.code);
