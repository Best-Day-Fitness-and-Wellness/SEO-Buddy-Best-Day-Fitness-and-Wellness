...
function providerHealth(snapshot = {}, credentialMetadata = {}) {
  return Object.entries(PROVIDERS).map(([key, definition]) => {
    ...
    let detail = 'Configured, but no successful request is recorded for this credential.';
    ...
    if (!configured) {
      ...
      state = 'healthy';
      label = 'Working';
      detail = 'A successful request is recorded for this connection.';
    ...
    return {
      ...
      credentialUpdatedAt: credentialMetadata?.providers?.[key]?.lastReplacedAt || null,
    };
  });
}
...
function buildOperationalHealth(input = {}) {
  ...
  const integrations = providerHealth(input.providerSnapshot?.providers || input.providerSnapshot || {}, input.credentialMetadata);
