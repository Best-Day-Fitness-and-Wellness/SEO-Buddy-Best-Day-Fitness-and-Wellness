...
function createProviderRuntime(options = {}) {
  ...
  let stateObserver = typeof options.onStateChange === 'function' ? options.onStateChange : null;
  ...
  function notifyStateChange() {
    ...
    try { stateObserver(durableSnapshot()); }
    catch (error) { logger?.warn?.('provider.health_persist_failed', { error }); }
  }

  function setStateObserver(observer) {
    stateObserver = typeof observer === 'function' ? observer : null;
  }

  function hydrate(snapshot = {}) {
    if (!snapshot || snapshot.version !== 1 || !snapshot.providers || typeof snapshot.providers !== 'object') return false;
    const numericKeys = ['totalCalls', 'successes', 'failures', 'retries', 'cacheHits', 'consecutiveFailures', 'lastLatencyMs'];
    const dateKeys = ['lastAttemptAt', 'lastSuccessAt', 'lastFailureAt'];
    for (const [name, saved] of Object.entries(snapshot.providers)) {
      if (!/^[a-z][a-z0-9-]{0,79}$/.test(name) || !saved || typeof saved !== 'object') continue;
      ...
      for (const key of dateKeys) {
        state[key] = Number.isFinite(Date.parse(saved[key])) ? new Date(saved[key]).toISOString() : null;
      }
      const openedAt = Number(saved.circuitOpenedAt);
      state.circuitOpenedAt = Number.isFinite(openedAt) && openedAt > 0 ? openedAt : null;
    }
    return true;
  }
  ...
  async function run(provider, operation, runOptions = {}) {
    ...
    try {
      ...
      state.lastLatencyMs = Math.max(0, clock() - startedAt);
      state.lastError = publicProviderError(lastError, { provider: state.provider }).code;
  ...
  return { clearCache, durableSnapshot, fetch: fetchWithPolicy, hydrate, reset, run, setConfigured, setStateObserver, snapshot };
}
