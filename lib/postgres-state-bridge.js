...
function createPostgresStateBridge(options) {
  ...
  async function runDrain() {
    while (!closed && Object.keys(state.entries).length) {
      const batch = Object.fromEntries(Object.entries(state.entries).map(([key, entry]) => [key, entry.value]));
      const sequences = Object.fromEntries(Object.entries(state.entries).map(([key, entry]) => [key, entry.sequence]));
      try {
        if (store.putStates) await store.putStates(repository.tenantId, batch);
        else for (const [key, value] of Object.entries(batch)) await store.putState(repository.tenantId, key, value);
        for (const [key, sequence] of Object.entries(sequences)) {
          if (state.entries[key]?.sequence === sequence) delete state.entries[key];
        }
        persistOutbox(filePath, state);
        ...
        logger?.error?.('storage.postgres_write_failed', { tenantId: repository.tenantId, stateKeys: Object.keys(batch), error });
