...
const ASSISTANT_TOOLS = [{
  functionDeclarations: [
    { name: 'open_walkthrough', description: 'Offer the read-only five-step owner walkthrough when the user asks for a tour, walkthrough, or introduction to the app. Opens only after they click Start walkthrough; no scans, publishing, or settings changes.', parameters: { type: 'OBJECT', properties: {} } },
...
- optimizationScore is the current dashboard calculation; if unavailable, do not substitute a historical score. Connection booleans mean configured, not proof of a successful live request. Distinguish scheduled work from completed work. googlePost owner-marked means manually recorded by the owner, NOT verified by Google; google-confirmed means Google returned a post receipt. For Business Profile publishing, use connections.googleBusinessProfile.status and approval details to identify the exact next step; a recorded approval status is owner-supplied tracking, not proof from Google. For report questions use monthlyReport readiness, masked recipient, lastSentAt and nextRunAt; a recorded send is not proof the recipient read the email. Only claim AI coverage for enginesConnected entries with connected=true.
- For "is everything working" questions, use systemHealth.overall and its current alerts. If systemHealth.status is unavailable, say the check is unavailable rather than claiming an all-clear. failureAlerts describes the owner's email preference and delivery readiness; enabling it does not send a test email. Direct the owner to Settings → Your connections → System health to change it. Never claim that a configured address proves an alert was delivered.
...
- Never claim you will run, post, or send anything unless calling the matching tool in this turn. You may describe the verified navigation below without a tool, but never invent controls or claim an action already happened.
- For actions you have no tool for (publishing a full article, sending email), explain briefly and point them to the right tab.
- If someone asks for a tour or walkthrough, call open_walkthrough. The redesigned workspace also has Help → Start the walkthrough below every page header. It visits the real Today, score, Approvals, Results, and Connections screens and spotlights the section being explained. It is view-only: no publishing or settings changes. Progress is saved in this browser; Finish or Skip returns to the starting page and preserves drafts. Do not refer to a top-bar question-mark icon or a Quick Guide. If using the recovery interface, the user must return to the redesigned workspace first.
...
function assistantPublicError(error) {
  ...
  if ([401, 403].includes(status) || /unauthenticated|invalid auth|api.?key.*invalid|access_token_type_unsupported|permission denied/.test(source)) {
    return {
  ...
  if (status === 429 || /resource_exhausted|rate.?limit|quota|billing/.test(source)) {
    return {
      code: 'ASSISTANT_USAGE_LIMIT_REACHED',
      error: 'Gemini is not accepting more requests right now. Check the Gemini account billing and usage limits, then try again.',
    };
  }
  if (error?.name === 'AbortError' || error?.code === 'PROVIDER_TIMEOUT' || /timed? ?out|timeout|econnreset|enetwork|enotfound|eai_again/.test(source)) {
    return {
      code: 'ASSISTANT_PROVIDER_TIMEOUT',
      error: 'Gemini did not respond in time. Your SEO Buddy data was not changed; please try again in a moment.',
    };
  }
  return {
...
function resolveAssistantAction(name, args, context = {}) {
  args = args || {};
  switch (name) {
    case 'open_walkthrough': return { kind: 'help', id: name, clientAction: 'walkthrough', title: 'Get to know SEO Buddy', note: 'Five short steps. No scans, publishing, or settings changes.', confirmLabel: 'Start walkthrough' };
  ...
  app.post('/api/assistant', requireAuth, async (req, res) => {
    ...
    try {
      ...
      const reply = text.trim() || (action
        ? (action.kind === 'help' ? 'Select **Start walkthrough** to visit the real pages and see each section highlighted. Finish or Skip returns you to your starting page; drafts are preserved.' : action.kind === 'content'
      ...
      logger.error('[Assistant] failed:', publicError.code);
      return res.status(502).json({ success: false, ...publicError });
    }
  });
}
