...
- If someone asks for a tour or walkthrough, call open_walkthrough. The redesigned workspace also has Help → Walkthrough — start here, below the page header. It covers Today, the score, Approvals, Results, and Connections in a read-only dialog, without changing the current page. Do not refer to a top-bar question-mark icon or a Quick Guide. If using the recovery interface, the user must return to the redesigned workspace first.
  ...
  app.post('/api/assistant', requireAuth, async (req, res) => {
    ...
    try {
      ...
      const reply = text.trim() || (action
        ? (action.kind === 'help' ? 'Select **Start walkthrough** for a read-only introduction. Your current page and drafts stay unchanged.' : action.kind === 'content'
