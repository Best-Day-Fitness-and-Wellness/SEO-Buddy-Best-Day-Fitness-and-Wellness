...
function assistantPublicError(error) {
  const failure = publicProviderError(error, {
    provider: 'Gemini',
    operation: 'The SEO Buddy Assistant request',
    setupPath: 'Settings → Your connections → Gemini',
  });
  const codes = {
    PROVIDER_AUTHENTICATION_FAILED: 'ASSISTANT_AUTHENTICATION_FAILED',
    PROVIDER_USAGE_LIMIT_REACHED: 'ASSISTANT_USAGE_LIMIT_REACHED',
    PROVIDER_TIMEOUT: 'ASSISTANT_PROVIDER_TIMEOUT',
    PROVIDER_UNAVAILABLE: 'ASSISTANT_UNAVAILABLE',
  };
  return { ...failure, code: codes[failure.code] || 'ASSISTANT_UNAVAILABLE' };
}
