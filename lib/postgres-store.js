...
function createPostgresStore(options) {
  ...
  async function putState(tenantId, key, value) {
    await pool.query(`INSERT INTO tenant_state (tenant_id, state_key, payload, version, updated_at)
  ...
  return { close: () => pool.end(), getState, health, migrate, pool, putState, syncFrom };
}
