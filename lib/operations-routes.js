...
function registerOperationsRoutes(app, options) {
  ...
  app.get('/api/integration-health', requireAuth, async (req, res) => {
    try {
      const budget = getBudget();
      const providerSnapshot = providerRuntime.snapshot();
      const overview = await getOperationalHealth({ budget, providerSnapshot });
      res.json({ success: true, budget, ...providerSnapshot, overview });
    } catch (_) {
      res.status(503).json({ success: false, error: 'Unable to verify system health. Try again shortly.' });
    }
  });
  ...
  app.get('/api/job-queue', requireAuth, async (req, res) => {
    try {
      res.json({
        success: true,
        tenantId: stateRepository.tenantId,
        backend: durableJobQueue.backend(),
        worker: { running: isJobWorkerRunning() },
        ...(await durableJobQueue.snapshot()),
      });
    } catch (error) {
