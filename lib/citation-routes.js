...
function registerCitationRoutes(app, options) {
  ...
  app.post('/api/citation-targets', requireAuth, async (req, res) => {
    ...
    try {
      ...
      const failure = publicProviderError(error, { provider: 'Gemini', operation: 'The citation analysis', setupPath: 'Settings → Your connections → Gemini' });
      logger.error('[Citation Targets] failed:', failure.code);
  ...
  app.post('/api/listing-kit', requireAuth, async (req, res) => {
    ...
    try {
      ...
      const failure = publicProviderError(error, { provider: 'Gemini', operation: 'The Listing Kit update', setupPath: 'Settings → Your connections → Gemini' });
      logger.error('[Listing Kit] regenerate failed:', failure.code);
  ...
  app.post('/api/citation-scan', requireAuth, async (req, res) => {
    ...
    try {
      ...
      const failure = publicProviderError(error, { provider: 'Gemini', operation: 'The citation scan', setupPath: 'Settings → Your connections → Gemini' });
      logger.error('[Citation Scan] failed:', failure.code);
      return res.status(502).json({ success: false, error: failure.error });
    }
  });
    ...
    if (LISTING_TYPES.includes(t)) {
      ...
      if (hasGeminiKey()) {
        try {
          ...
          logger.error('[Outreach listing] grounding failed:', publicProviderError(error, {
            provider: 'Gemini',
            operation: 'The listing link search',
...
Return ONLY raw JSON, no markdown: {"email":"the best real, publicly-listed email address, or empty string if none is published","contactUrl":"the URL to submit/pitch or the site's contact page (empty if none)","to":"a short human label for who this reaches, e.g. 'Features editor'","subject":"","body":"","howToFind":"one short line on how to reach or confirm the right recipient"}`;
      ...
      const failure = publicProviderError(error, { provider: 'Gemini', operation: 'The outreach draft', setupPath: 'Settings → Your connections → Gemini' });
      logger.error('[Outreach pitch] failed:', failure.code);
      return res.status(502).json({ success: false, error: failure.error });
    }
  });
}
