...
function createGscService(options) {
    ...
    if (auth && siteUrl) {
      try {
        ...
        return { source: 'live_gsc', data: mapQueryRows(response.data.rows || []),
          period: { startDate, endDate }, queryRowLimit: 100 };
      } catch (error) {
        const failure = publicProviderError(error, {
          provider: 'Google Search Console',
          operation: 'The search performance request',
          setupPath: 'Settings → Your connections → Google Search Console',
        });
        logger.error('[GSC API] Failed:', failure.code);
  ...
  async function getPages(query) {
    ...
    try {
      ...
      const failure = publicProviderError(error, {
        provider: 'Google Search Console',
        operation: 'The page-level search request',
        setupPath: 'Settings → Your connections → Google Search Console',
      });
      logger.error('[GSC API] Page query failed:', failure.code);
      return { source: 'error', query: q || null, code: failure.code, error: failure.error, data: [] };
    }
  }
...
function registerGscRoutes(app, options) {
  ...
  app.get('/api/gsc-data', async (req, res) => {
    try {
      ...
      res.status(502).json({ source: 'error', code: failure.code, error: failure.error, data: [] });
    }
  });
