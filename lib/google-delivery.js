...
const GBP_ACCESS_STATES = new Set(['not-requested', 'pending', 'approved', 'declined']);
...
function createGoogleDelivery(options) {
  ...
  function gbpReadiness() {
    const configured = gbpConfigured();
    const recordedAccess = String(env.GBP_API_ACCESS_STATUS || '').trim().toLowerCase();
    const approvalStatus = GBP_ACCESS_STATES.has(recordedAccess)
      ? recordedAccess
      : (configured ? 'approved' : 'not-recorded');
    const oauthAuthorized = !!env.GBP_REFRESH_TOKEN;
    const accountSelected = !!env.GBP_ACCOUNT_ID;
    const locationSelected = !!env.GBP_LOCATION_ID;
    let status = 'needs-approval';
    if (configured) status = 'ready';
    else if (approvalStatus === 'pending') status = 'pending-approval';
    else if (approvalStatus === 'declined') status = 'approval-declined';
    else if (approvalStatus === 'approved') status = 'needs-authorization';
    return {
      configured,
      status,
      approval: {
        status: approvalStatus,
        caseId: String(env.GBP_API_CASE_ID || '').trim().slice(0, 120) || null,
        submittedAt: String(env.GBP_API_SUBMITTED_AT || '').trim().slice(0, 10) || null,
      },
      steps: {
        apiAccessApproved: configured || approvalStatus === 'approved',
        googleAccountAuthorized: oauthAuthorized,
        businessAccountSelected: accountSelected,
        locationSelected,
      },
    };
  }
  ...
  return { gmailClient, sendGmail, gbpAuth, gbpConfigured, gbpReadiness, postGbpLocalPost };
}
