...
function registerLocalSeoRoutes(app, options) {
  ...
  app.post('/api/nap-audit', requireAuth, async (req, res) => {
    ...
    try {
      ...
      const failure = publicProviderError(error, { provider: 'Gemini', operation: 'The local listing check', setupPath: 'Settings → Your connections → Gemini' });
      logger.error('[NAP Audit] failed:', failure.code);
  ...
  app.post('/api/local-generate', requireAuth, async (req, res) => {
    ...
    try {
      ...
      const failure = publicProviderError(error, { provider: 'Gemini', operation: 'The local content request', setupPath: 'Settings → Your connections → Gemini' });
      logger.error('[Local Generate] failed:', failure.code);
      return res.status(502).json({ success: false, error: failure.error });
    }
  });
  ...
  app.post('/api/local-reply', requireAuth, async (req, res) => {
    ...
    try {
      ...
      const failure = publicProviderError(error, { provider: 'Gemini', operation: 'The review reply request', setupPath: 'Settings → Your connections → Gemini' });
      logger.error('[Local Reply] failed:', failure.code);
      return res.status(502).json({ success: false, error: failure.error });
    }
  });
}
