  ...
  async function computePerformanceSnapshot() {
    ...
    if (auth && siteUrl) {
      try {
        ...
        logger.error('[Performance] GSC failed:', publicProviderError(error, {
          provider: 'Google Search Console',
          operation: 'The performance comparison',
    ...
    if (token && locationId) {
      try {
        ...
        const failure = publicProviderError(error, {
          ...
          operation: 'The lead attribution request',
          setupPath: 'Settings → Your connections → GoHighLevel',
        });
        logger.error('[Performance] GoHighLevel failed:', failure.code);
        out.leads = { available: false, reason: failure.error, code: failure.code };
      }
    } else {
...
function registerPerformanceRoutes(app, options) {
  ...
  app.get('/api/performance', async (req, res) => {
    try {
      ...
      const failure = publicProviderError(error, {
        provider: 'the reporting service',
        operation: 'The performance report',
      });
      res.status(500).json({ success: false, code: failure.code, error: failure.error });
    }
  });
}
