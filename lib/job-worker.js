...
function createJobWorker(options) {
  ...
  const reportHeartbeat = options.reportHeartbeat || (async () => {});
  ...
  async function heartbeat() {
    try { await reportHeartbeat({ workerId, activeJobId: activeJob?.id || null }); }
    catch (error) { logger.warn('job.worker_heartbeat_failed', { workerId, error }); }
  }
  ...
  function start() {
    ...
    timer.unref?.();
    heartbeatTimer = setInterval(heartbeat, Math.max(5000, Math.min(30000, heartbeatMs)));
