...
const PRESERVED_SETTINGS = [
  ...
  'GBP_API_ACCESS_STATUS', 'GBP_API_CASE_ID', 'GBP_API_SUBMITTED_AT',
];

const CREDENTIAL_INPUTS = Object.freeze([
  { field: 'geminiKey', setting: 'GEMINI_API_KEY', provider: 'gemini', label: 'Gemini API key' },
  { field: 'openaiKey', setting: 'OPENAI_API_KEY', provider: 'openai', label: 'OpenAI API key' },
  { field: 'perplexityKey', setting: 'PERPLEXITY_API_KEY', provider: 'perplexity', label: 'Perplexity API key' },
  { field: 'ghlToken', setting: 'GHL_ACCESS_TOKEN', provider: 'gohighlevel', label: 'GoHighLevel access token' },
]);

const MANAGED_SECRET_SETTINGS = new Set([
  'GEMINI_API_KEY', 'OPENAI_API_KEY', 'PERPLEXITY_API_KEY', 'GHL_ACCESS_TOKEN',
  'GOOGLE_APPLICATION_CREDENTIALS', 'ADMIN_PASSWORD', 'OPERATOR_PASSWORD',
  'AUDIT_SIGNING_KEY', 'TRUSTPILOT_API_KEY',
]);
...
function normalizeSettings(input, environment, options = {}) {
  const persistSecrets = options.persistSecrets !== false;
  const saved = {};
  for (const key of PRESERVED_SETTINGS) {
    if (!persistSecrets && MANAGED_SECRET_SETTINGS.has(key)) continue;
  ...
  if (Object.prototype.hasOwnProperty.call(input, 'gbpAccessStatus')) {
    const status = cleanSettingValue(input.gbpAccessStatus, 32).toLowerCase() || 'not-requested';
    saved.GBP_API_ACCESS_STATUS = status;
    const caseId = cleanSettingValue(input.gbpCaseId, 120);
    const submittedAt = cleanSettingValue(input.gbpSubmittedAt, 10);
    if (caseId) saved.GBP_API_CASE_ID = caseId; else delete saved.GBP_API_CASE_ID;
    if (submittedAt) saved.GBP_API_SUBMITTED_AT = submittedAt; else delete saved.GBP_API_SUBMITTED_AT;
    if (status === 'not-requested') {
      delete saved.GBP_API_CASE_ID;
      delete saved.GBP_API_SUBMITTED_AT;
    }
  }
  return saved;
}

function credentialReplacements(input, environment) {
  const changed = CREDENTIAL_INPUTS.filter(item => {
    const replacement = normalizeSecretInput(input[item.field], item.label);
    return replacement && replacement !== environment[item.setting];
  }).map(item => item.provider);
  if (cleanSettingValue(input.gscJson, 2 * 1024 * 1024)) changed.push('search-console');
  return changed;
}

function validateSavedSettings(saved) {
  if (saved.GBP_API_ACCESS_STATUS && !['not-requested', 'pending', 'approved', 'declined'].includes(saved.GBP_API_ACCESS_STATUS)) {
    return 'Choose a valid Google Business Profile approval status.';
  }
  if (saved.GBP_API_CASE_ID && !/^[a-z0-9-]+$/i.test(saved.GBP_API_CASE_ID)) {
    return 'The Google support case ID can contain only letters, numbers, and dashes.';
  }
  if (saved.GBP_API_SUBMITTED_AT && !/^\d{4}-\d{2}-\d{2}$/.test(saved.GBP_API_SUBMITTED_AT)) {
    return 'The Google application date must use YYYY-MM-DD.';
  }
...
function registerConfigurationRoutes(app, options) {
  const {
    ...
    logger = console,
    secretStorageMode = 'volume',
  } = options;
  if (!['volume', 'managed'].includes(secretStorageMode)) throw new Error('secretStorageMode must be volume or managed.');

  app.get('/api/configuration-policy', requireOwner, (req, res) => res.json({
    success: true,
    secretStorage: {
      mode: secretStorageMode,
      editableInApp: secretStorageMode !== 'managed',
      configured: Object.fromEntries(CREDENTIAL_INPUTS.map(item => [item.provider, Boolean(environment[item.setting])])),
      googleServiceAccountConfigured: Boolean(environment.GOOGLE_APPLICATION_CREDENTIALS),
    },
  }));
  ...
  app.post('/api/save-settings', requireOwner, (req, res) => {
    ...
    try {
      const changedCredentials = credentialReplacements(input, environment);
      const hasGoogleCredential = Boolean(cleanSettingValue(input.gscJson, 2 * 1024 * 1024));
      if (secretStorageMode === 'managed' && (changedCredentials.length || hasGoogleCredential)) {
        return res.status(409).json({
          success: false,
          error: 'Credentials are managed by the deployment secret store. Update them in Railway Variables, then redeploy.',
          code: 'MANAGED_SECRETS_REQUIRED',
        });
      }
      const saved = normalizeSettings(input, environment, { persistSecrets: secretStorageMode !== 'managed' });
      ...
      clearCaches();
      onCredentialsChanged(changedCredentials);

      return res.json({
        success: true,
        persistent: !!environment.DATA_DIR,
        credentialsUpdated: changedCredentials,
        secretStorageMode,
