...
function registerRecordedContentRoutes(app, options) {
  ...
  app.post('/api/transcribe', requireAuth, async (req, res) => {
    try {
      ...
      if (!transcript) {
        logger.error('[Transcribe] failed:', 'PROVIDER_EMPTY_RESPONSE');
        return res.status(500).json({
          success: false,
          error: 'Nothing came back from the transcription — try a shorter or clearer recording.',
        });
      }
      ...
      const failure = publicProviderError(error, { provider: 'Gemini', operation: 'The transcription', setupPath: 'Settings → Your connections → Gemini' });
      logger.error('[Transcribe] failed:', failure.code);
      return res.status(500).json({ success: false, error: failure.error });
    }
  });
  ...
  app.post('/api/social-pack', requireAuth, async (req, res) => {
    try {
      ...
      if (!pack.script) {
        logger.error('[Social pack] failed:', 'PROVIDER_EMPTY_RESPONSE');
        return res.status(500).json({
          success: false,
          error: 'Gemini did not return a usable script — try again.',
        });
      }

      return res.json(shapeSocialPack(pack, ideaIndex, hookIndex));
    } catch (error) {
      const failure = publicProviderError(error, { provider: 'Gemini', operation: 'The social post pack', setupPath: 'Settings → Your connections → Gemini' });
      logger.error('[Social pack] failed:', failure.code);
      return res.status(500).json({ success: false, error: failure.error });
    }
  });
}
