...
function registerOnsiteRoutes(app, options) {
  ...
  app.post('/api/onsite', requireAuth, async (req, res) => {
    ...
    try {
      ...
      const failure = publicProviderError(error, { provider: 'Gemini', operation: 'The site optimization request', setupPath: 'Settings → Your connections → Gemini' });
      logger.error('[On-Site] failed:', failure.code);
