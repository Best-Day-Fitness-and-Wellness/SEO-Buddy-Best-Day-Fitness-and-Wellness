...
1. **`server.js` is still the main monolith.** Operations, profile, usage,
   ...
   AI audits, scheduled feature controls, and Google delivery are extracted;
   continue one route family at a time into services, integrations, and repository
   interfaces. Keep contract tests around the existing HTTP boundary before every
   extraction.
