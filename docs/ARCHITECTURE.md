...
social-pack interactions, `pdf-report.js` owns report assembly and PDF-library
loading, and `theme.js` owns theme behavior. `public/app.js` remains the main
feature coordinator.

The browser loads only the visible dashboard work initially. Feature loaders
use same-origin APIs and render escaped values. PDF reporting, Reviews, and
recorded-content modules load on demand; the PDF vendor libraries remain behind
...
4. **`public/app.js` is still large.** Reviews, recorded content, PDF reporting, assistant, theme, and core are
   separate modules; Reviews, recorded content, and PDF reporting are lazy loaded. Continue feature-by-feature extraction
