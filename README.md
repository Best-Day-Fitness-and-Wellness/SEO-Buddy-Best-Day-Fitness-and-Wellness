...
| `PROCESS_ROLE` | `all` | `all` runs HTTP, schedules and workers together. At scale, use one `web` service to serve/schedule and one or more `worker` services to claim PostgreSQL jobs. Worker services expose health probes only. |
| `SECRET_STORAGE_MODE` | `volume` | Set to `managed` after credentials are in Railway Variables. Managed mode disables credential editing in the app and rejects secret writes while preserving non-secret settings. |
...
- In `volume` mode, API keys entered in Settings are sent once to the server, cleared from the form after a successful save, and never retained in browser local storage. In `managed` mode, credentials stay in Railway Variables and cannot be read or replaced through the app. Non-secret business settings remain editable.
